...
import { PasswordProps, PasswordStrength } from "./password.props";
...
const slots = defineSlots<{
    ...
    header?: (props: {}) => any;
    /** Custom content at the bottom of the feedback panel */
    footer?: (props: { strength: PasswordStrength }) => any;
}>();

const props = withDefaults(defineProps<PasswordProps>(), {
    // Inherited from InputTextProps
    size: "medium",
    variant: "outline",
    invalid: false,
    fluid: false,
...
const onFocus = () => { overlayVisible.value = true; };
const onBlur = () => { overlayVisible.value = false; };

// Inline SVG render functions for default icons
...
const internalStrength = (val: string): PasswordStrength => {
...
const strength = computed(() => {
    ...
    return typeof props.strengthMeter === 'function'
        ? props.strengthMeter(val)
        : internalStrength(val);
});
...
const strengthLabel = computed(() => {
    ...
    switch(s) {
        case "weak": return props.weakLabel;
        case "medium": return props.mediumLabel;
        case "strong": return props.strongLabel;
        default: return props.promptLabel;
    }
});
</script>
