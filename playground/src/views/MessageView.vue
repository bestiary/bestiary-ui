<template>
    <div class="view-content">
        ...
        <section class="mt-8">
            ...
            <div class="section flex justify-center items-end gap-2">
        ...
        <section class="mt-8">
            <h4 class="mb-3">With input</h4>
            <div class="section flex flex-col items-center gap-2">
                <BMessage v-if="!username || !email" severity="error" :icon="XCircleOutline" class="mb-2">
                    Validation error
                </BMessage>
                <BMessage v-if="username && email" severity="success" :icon="XCircleOutline" class="mb-2">
                    Form is valid
                </BMessage>
                <div class="flex flex-col gap-1">
                    <BInputText v-model="username" placeholder="Username" aria-label="username" :invalid="!username"/>
                    <BMessage v-show="!username" severity="error" variant="simple" size="small">
                        Username is required
                    </BMessage>
                </div>
                <div class="flex flex-col gap-1">
                    <BInputText v-model="email" placeholder="Email" aria-label="email" :invalid="!email"/>
                    <BMessage v-show="!email" severity="error" variant="simple" size="small">
                        Email is not valid
                    </BMessage>
                </div>
            </div>
